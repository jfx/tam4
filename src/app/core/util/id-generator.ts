...
export class IdGenerator {
    ...
    public static generate() {
        ...
        const duplicateTime = (now === IdGenerator.lastPushTime);
        IdGenerator.lastPushTime = now;

        const timeStampChars = new Array(8);
        for (let i = 7; i >= 0; i--) {
            timeStampChars[i] = IdGenerator.PUSH_CHARS.charAt(now % 64);
        ...
        if (!duplicateTime) {
            for (let i = 0; i < 12; i++) {
                IdGenerator.lastRandChars[i] = Math.floor(Math.random() * 64);
            }
        } else {
            let i;
            // If the timestamp hasn't changed since last push, use the same random number, except incremented by 1.
            for (i = 11; i >= 0 && IdGenerator.lastRandChars[i] === 63; i--) {
